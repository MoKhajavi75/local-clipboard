...
**Browser auto-open:** On startup `main` binds the listener with `net.Listen` and then calls `openBrowser("http://localhost:<port>")` before `http.Serve`, so the socket is already queueing connections when the browser hits it. Launchers per platform: `open` (macOS), `rundll32 url.dll,FileProtocolHandler` (Windows), `xdg-open` (Linux). `canOpenBrowser` suppresses it inside containers (`/.dockerenv`) and on headless Linux (no `DISPLAY`/`WAYLAND_DISPLAY`). Users can disable with `-open=false` or `LOCAL_CLIPBOARD_NO_OPEN`. A launcher failure only logs.
